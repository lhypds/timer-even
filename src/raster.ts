...
function rasterizePixels(
  ...
  if (invert)
    gray.fill(255, layout.band.top * width, layout.band.bottom * width);
  if (text) {
    ...
    const x =
      layout.align === "left"
        ? layout.anchor.x
        : layout.align === "right"
          ? layout.anchor.x - textWidth
          : Math.round(layout.anchor.x - textWidth / 2);
    blit(
      gray,
      width,
      height,
      text,
      x,
      layout.anchor.baseline - PIXEL_HEIGHT,
      invert ? 0 : 255,
    );
  }
...
function rasterizeVector(
  ...
  if (invert) {
    ...
    ctx.fillRect(
      ...
      layout.band.top,
      width,
      layout.band.bottom - layout.band.top,
    );
  }
  if (text) {
...
export function rasterize(
  ...
  return layout.pixel
    ? rasterizePixels(text, layout, invert)
    : rasterizeVector(text, layout, invert);
}
