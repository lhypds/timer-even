...
export function parseState(data: unknown, session: string): TimerState | null {
  ...
  if (
    ...
    !(
      s.countTo === null ||
      (Number.isInteger(s.countTo) &&
        (s.countTo as number) >= 0 &&
        (s.countTo as number) <= 1439)
    )
  )
    return null;
...
export function secondsAt(state: TimerState, now: number): number {
  ...
  return Math.min(
    MAX_SECONDS,
    Math.max(0, state.seconds + (state.mode === "timer" ? -elapsed : elapsed)),
  );
}
...
export function timeText(
  ...
  if (state.countTo !== null)
    return `${pad(Math.floor(state.countTo / 60))}:${pad(state.countTo % 60)}`;
  ...
  return milliseconds && !hours
    ? `${base}.${pad(Math.floor((seconds % 1) * 100))}`
    : base;
}
