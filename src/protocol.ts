...
export function parseState(data: unknown, session: string): TimerState | null {
  ...
  if (s.source !== PROTOCOL || s.version !== 1 || s.type !== 'state' || s.session !== session ||
      !Number.isSafeInteger(s.sequence) || (s.sequence as number) < 0 ||
      (s.mode !== 'timer' && s.mode !== 'stopwatch') ||
      !finite(s.seconds) || s.seconds < 0 || s.seconds > MAX_SECONDS ||
      typeof s.running !== 'boolean' || !finite(s.sampledAt) || s.sampledAt <= 0 ||
      !(s.countTo === null || (Number.isInteger(s.countTo) && (s.countTo as number) >= 0 && (s.countTo as number) <= 1439))) return null;
