...
const base = {
  source: 'gcc3-timer', version: 1, type: 'state', session, sequence: 0,
  mode: 'timer', seconds: 300, running: true, sampledAt: 100000, countTo: null
};
...
const fakeRasterize = (text, l, invert) => l.tiles.map((_, i) => Uint8Array.of((text ? text.charCodeAt(i) : 0) + (invert ? 128 : 0), i));
...
test('reject malformed or incompatible messages', () => {
  ...
  for (const bad of [null, {}, { ...base, version: 2 }, { ...base, seconds: NaN },
    { ...base, seconds: Infinity }, { ...base, seconds: -1 }, { ...base, seconds: 360000 },
    { ...base, running: 'true' }, { ...base, mode: 'unknown' }, { ...base, sampledAt: NaN },
    { ...base, session: 'wrong' }, { ...base, sequence: -1 }, { ...base, countTo: 1440 }]) {
    assert.equal(parseState(bad, session), null);
  }
});

test('settings fall back field by field', () => {
  assert.deepEqual(DEFAULT_SETTINGS, { milliseconds: false, position: 'left-top', size: 'tiny', blink: 'text' });
  assert.deepEqual(parseSettings(null), DEFAULT_SETTINGS);
  assert.deepEqual(parseSettings('x'), DEFAULT_SETTINGS);
  assert.deepEqual(
    parseSettings({ size: 'huge', position: 'right-top', blink: 'yes', milliseconds: true }),
    { milliseconds: true, position: 'right-top', size: DEFAULT_SETTINGS.size, blink: 'text' }
  );
  assert.deepEqual(parseSettings({ size: 'tiny', position: 'center', blink: 'background', milliseconds: true }),
    { milliseconds: true, position: 'center', size: 'tiny', blink: 'background' });
...
test('URL overrides hold for one launch and ignore junk', () => {
  const saved = { milliseconds: false, position: 'left-top', size: 'big', blink: 'none' };
...
test('sizes step down below the native face and every bitmap fits the display', () => {
  ...
  assert.deepEqual(TINY.band, { top: TINY.anchor.baseline - PIXEL_HEIGHT - 2, bottom: TINY.anchor.baseline + 2 },
  ...
  assert.ok(medium.band.top >= 0 && medium.band.bottom <= medium.rect.height && medium.band.bottom - medium.band.top >= 0.72 * medium.font);
  ...
  assert.deepEqual(BIG.anchor, { x: BIG.rect.width - 2, baseline: Math.round((84 + 0.72 * 84) / 2) }, 'right-aligned, digits centred in the box');
});
...
test('the pixel face draws crisp dots without a canvas', () => {
  ...
  const rows = new Set();
  tile.forEach((b, i) => { if (b) rows.add(Math.floor(i / TINY.rect.width)); });
  ...
  left.forEach((b, i) => { if (b) columns.add(i % leftLayout.rect.width); });
...
function senderHarness(origin = 'https://app.example') {
  ...
  const host = {
    location: { search: `?evenSession=${session}&evenParentOrigin=${encodeURIComponent(origin)}` }, parent,
    addEventListener: (type, listener) => { receive = listener; },
    removeEventListener: () => { receive = null; }
  };
  let snapshot = { ...base };
  const commands = [];
  const sender = createEvenTimerSender(host, () => snapshot, command => commands.push(command));
  const request = { source: 'gcc3-timer-even', version: 1, type: 'request-state', session };
  const say = (data, overrides = {}) => receive({ source: parent, origin, data, ...overrides });
  return { sender, sent, parent, commands, set: value => { snapshot = value; },
...
test('handshake restricts source, origin and session; sender is inert until connected', () => {
  ...
  h.request({ source: {} });
  h.request({ data: { source: 'gcc3-timer-even', version: 1, type: 'request-state', session: 'wrong' } });
...
test('a toggle from the glasses is honoured only from the connected companion', () => {
  ...
  h.toggle({ source: {} });
  h.toggle({ data: { source: 'gcc3-timer-even', version: 1, type: 'toggle', session: 'wrong' } });
  h.toggle({ data: { source: 'gcc3-timer-even', version: 1, type: 'pause', session } });
...
test('a re-created sender continues above its predecessor, so a remount is not old news', async () => {
  ...
  first.request();
  first.set({ ...base, seconds: 299 }); first.sender.publish();
  first.set({ ...base, seconds: 298 }); first.sender.publish();
...
test('slow Bluetooth writes serialize and coalesce to the latest time', async () => {
  ...
  const display = new TimerDisplay({
    create: async f => { calls.push(f.text); await new Promise(r => { release = r; }); },
    update: async f => { calls.push(f.text); }
  });
...
test('a refused update rebuilds the page; reconnect resends paused text', async () => {
  ...
  const display = new TimerDisplay({
    ...
    update: async () => { updates++; if (updates === 1) throw Error('Disconnected'); }
  });
...
test('the blink keeps the phone\'s rate on a quick link and never skips a phase on a slow one', () => {
  ...
  for (let now = 0; now <= 6000; now += 50) {
    ...
    if (phase !== last) { flips.push(now); landsAt = now + 800; last = phase; }
  }
  const gaps = flips.slice(1).map((t, i) => t - flips[i]);
  assert.ok(gaps.every(g => g >= 800 && g <= 850), `each phase lasts one frame, not two: ${gaps}`);
...
test('a display is settled once the frame asked for is the one on the glass', async () => {
  let release;
  const display = new TimerDisplay({
    create: async () => { await new Promise(r => { release = r; }); },
    update: async () => {}
  });
...
test('a changed layout rebuilds the page once; blank and inverted frames are writes', async () => {
  const calls = [];
  const display = new TimerDisplay({
    create: async f => { calls.push(['create', f.layout.font, f.text]); },
    update: async f => { calls.push(['update', f.layout.font, f.text, f.invert ?? false]); }
  });
  await display.paint(frame('05:00'));
  ...
  assert.deepEqual(calls, [
    ['create', 7, '05:00'], ['create', 84, '05:00'], ['update', 84, '04:59', false],
    ['update', 84, '', false], ['update', 84, '04:59', false], ['update', 84, '04:59', true],
    ['update', 84, '04:59', false], ['create', 7, '04:59']
  ]);
});
...
function fakeBridge(startResults = [0]) {
  ...
  return {
    calls,
    refuseImages(value) { refuse = value; },
    ...
    async rebuildPageContainer(page) {
      calls.push(['rebuild', page.containerTotalNum, page.textObject[0].content,
...
test('a refused start-up page is retried; every build after it is a rebuild', async () => {
  ...
  assert.deepEqual(bridge.calls, [
    ['start', 1, CAPTURE],
    ['start', 1, CAPTURE],
    ['rebuild', 2, ' ', [[2, TINY.rect.width, TINY.rect.height, 1]]], ['image', 2, [48, 0]],
    ['image', 2, [49, 0]],
    ['rebuild', 3, ' ', BIG.tiles.map((t, i) => [2 + i, t.width, t.height, 1 + i])], ['image', 2, [48, 0]], ['image', 3, [52, 1]]
  ]);
});
...
test('a start-up page the host already has is rebuilt rather than retried', async () => {
  ...
  const transport = glassesTransport(bridge, fakeRasterize);
  await transport.create(frame('05:00'));
  assert.deepEqual(bridge.calls.map(c => c[0]), ['start', 'rebuild', 'image']);
});
...
test('ticks send only the tiles whose bytes moved; a refused tile is offered again', async () => {
  ...
  assert.deepEqual(bridge.calls.map(c => c[2]), [[0, 0], [0, 1]], 'a blank frame darkens every tile');
  bridge.calls.length = 0;
  ...
  assert.deepEqual(bridge.calls.map(c => c[1]), [2, 3], 'after a refusal every tile is sent again');
  bridge.calls.length = 0;
  await transport.update({ ...frame('25:00', BIG), invert: true });
  assert.deepEqual(bridge.calls.map(c => c[2]), [[50 + 128, 0], [53 + 128, 1]], 'an inverted frame is new bytes for every tile');
});
